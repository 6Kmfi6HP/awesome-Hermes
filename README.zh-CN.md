...
- [hermes-pwa](https://github.com/stasstepv/hermes-pwa) —— 非官方移动端 PWA，作为 Dashboard 插件安装后可添加 Mobile 标签页，可在手机上聊天、审批操作和监控 Agent。
...
- [Hermes Browser Extension](https://github.com/abundantbeing/hermes-browser-extension) —— Chromium 侧边栏扩展，将当前浏览器上下文连接到你本地或远程的 Hermes 运行时，支持侧栏聊天、标签页感知提示、语音输入和主题。作者 Jon Komet。
...
- [SOUL.md Grader Skill](https://github.com/cobibean/soul-grader-skill) —— 一键安装的技能，用于评分、审查和重写 Hermes Agent SOUL.md 身份文件，附带基于研究的 100 分评分标准和公共安全指南。
- [Interest Radar](https://github.com/Igloo302/interest-radar) —— 纯 SKILL.md 定义的相关性判断引擎，判断外部内容与你的兴趣相关度、原因及是否值得阅读，带反馈闭环越用越准；平台无关，零代码依赖。
- [Founder OS](https://github.com/wolulabs/hermes-founder-os) —— 模块化 Hermes Agent 技能包，将原始创业想法转化为经过验证、可构建的 SaaS 工作流，把构思、市场调研、产品规划、编码、UX 评审和发布拆解为专注的专项技能。
- [B2B SDR Skill](https://github.com/iPythoning/b2b-sdr-hermes-skill) —— 自主 AI SDR 技能，通过 WhatsApp、Email 和 Telegram 寻找买家、筛选线索并管理外联，面向 B2B 出口业务。
- [Threads Growth Skill](https://github.com/krumjahn/threads-growth-skill) —— 基于真实数据的 Threads 内容优化技能，提供个性化策略和效果分析，系统提升社交影响力。
- [Skills Judgment](https://github.com/zzyong24/skills-judgment) —— 自持续技能生命周期管理，利用自然选择原则保持 Hermes 技能生态的精简、相关与自动清理。
- [Link Bookmark Skill](https://github.com/PeterCha90/link-bookmark) —— 从聊天频道捕获未读链接书签，按频道分组整理，方便后续查阅。
- [Android Phone Control Skill](https://github.com/patrickdass93/android-phone-control-skill) —— 通过 ADB 安全控制 Android 设备的 Hermes 技能，支持屏幕状态、通话、短信、通知和设备信息操作，带审批门控。
...
- [hermes-patchkit](https://github.com/Anry777/hermes-patchkit) —— 在保留官方 Hermes Agent 上游代码的同时，以可复用补丁包形式应用自己的修复和定制。
...
- [Hermes Cost Lens](https://github.com/JDemler/hermes-cost-lens) —— 零构建 Dashboard 插件，用于分析和优化 Hermes Agent 会话成本，集成实时 OpenRouter 定价、D3 可视化图表，可在 Dashboard 会话页面直接查看每次会话的详细成本 breakdown。
...
- [Hermes-MPM](https://github.com/davidgut1982/hermes-mpm) —— 多 Profile 管理插件，带分层模型路由（纯 Python 确定性分类器）、并行编排器（单次批量委派调用的扇出）、意图快速路径（简单查询绕过 LLM）和仪表盘子代理运行追踪面板。灵感来自 claude-mpm。
- [Hermes Paperclip Cockpit](https://github.com/4crimson/hermes-paperclip-cockpit) —— 将 Telegram 变成 Paperclip 确定性控制面的 Hermes 插件，提供斜杠命令视图、自然语言重写钩子和保守的安全默认值，避免 Paperclip 控制逻辑膨胀模型上下文。
- [Hermes Plugin: Obscura Browser](https://github.com/SGavrl/hermes-plugin-obscura) —— 轻量级浏览器后端插件，在 Rust 编写的 Obscura 无头浏览器上通过 CDP 运行 Hermes 浏览器任务，约 70 MB 二进制和 30 MB 内存/会话，无需 Chrome 或 Node.js。
- [Hermes Forge](https://github.com/iknowkungfubar/hermes-forge) —— 自托管 MCP 服务器和 Hermes 插件，将 Forge 护栏框架引入 Hermes：响应验证、救助解析、步骤强制和上下文管理，让 LLM 工具调用更安全。
- [Sensorium](https://github.com/TKCen/sensorium) —— 环境反应式注意力基底插件，为 Hermes 添加受限的生命周期内层：显著性感知、潜意识/建议审查、意识线程胶囊和安全动作准备，无自主对外行为。
- [Shark Trading Agent](https://github.com/logiqfish/shark-trading-agent) —— 轻量自托管纯模拟交易机器人，作为 Hermes Agent Profile 分发运行，使用 Alpaca 模拟 API 获取市场数据并执行交易。只需两个密钥（Alpaca 模拟 + LLM），无需托管服务，仅用于教育目的。
...
- [Hermes Offline Installer](https://github.com/bddiudiu/hermes-offline-installer) —— 离线安装包构建器，将 Hermes Agent 与可移植 Python 运行时、uv、依赖包、仪表盘资源、技能和插件打包为一键安装程序，支持 Windows、macOS 和 Linux，安装过程无需 GitHub 或 PyPI 网络访问。
...
- [Hermes One-Click](https://github.com/Devsoul2026/Hermes-One-Click) —— Windows 版 Hermes 可视化桌面包，将 hermes-agent、hermes-webui、原生 WebView2 壳、便携 Python 运行时和内置技能打包为 Inno Setup 安装程序，一键部署无需配置开发环境。
- [HuggingMes](https://github.com/somratpro/HuggingMes) —— 面向 Hugging Face Spaces 的自托管 Hermes Agent 网关，可在免费套餐上 24/7 运行，支持 Cloudflare 代理外部流量、HF Dataset 持久备份、JupyterLab 终端和一键复制部署。
- [Render Hermes Agent 模板](https://github.com/render-examples/hermes-render) —— 官方一键 Render 部署模板，提供持久存储、仪表盘和 Docker Web 服务配置。
- [Telegram Bot 保活代理](https://github.com/vaibhavjnf/telegram-india-unblock) —— 当 ISP 封锁 api.telegram.org 时保持 Hermes Telegram Bot 在线的自愈方案，通过动态 SOCKS5 出口路由并自动回退。
- [Hermes Telegram 代理](https://github.com/alt-glitch/hermes-telegram-proxy) —— 基于 Cloudflare Worker 的加固反向代理，从封锁地区访问 Telegram Bot API 无需 VPN；Bot token 永不落日志。
- [HERMES-Free-SOTA](https://github.com/shagghiesuperstar/HERMES-Free-SOTA) —— 可复用的技能与运维指南，利用 FreeLLMAPI 的免费池配额以 Mixture-of-Agents 方式运行 Hermes Agent，零模型成本实现 SOTA 级 LLM 质量。已用 65 个模型、9 个模型族验证，内置 6 小时健康检查定时任务。
...
- [LiteLLM Agent 控制平面](https://github.com/LiteLLM-Labs/litellm-agent-control-plane) —— 统一控制平面，在一个界面中管理 Hermes、OpenCode、Claude Managed Agents、Cursor Agents API 和 DeepAgents，集成 agent vault、沙箱和 AI 网关路由。
- [Hermes LINE 桥接](https://github.com/hfujikawa77/hermes-line-bridge) —— 独立 LINE ↔ Hermes Agent 桥接，通过 OpenAI 兼容 API 服务器将 LINE 消息作为原生 Hermes 频道接入。
- [Zo-Hermes 桥接](https://github.com/JacKALaing/zo-hermes) —— Hermes Agent 的 Zo Computer 桥接方案，提供 Zo 兼容的 FastAPI 服务器和设置技能。
- [Cursor SDK for Hermes](https://github.com/AureliusIvan/cursor-sdk-for-hermes) —— 通过本地 OpenAI 兼容代理将 Cursor Composer 作为 Hermes 的模型提供者使用，附带声明式 Hermes 插件。
...
- [OMH — Oh My Hermes（多代理编排）](https://github.com/witt3rd/oh-my-hermes) —— 可组合的多代理编排技能集（91★）：共识规划（ralplan）、需求访谈（deep-interview）、验证执行（ralph）、多视角研究（deep-research）和全自动流水线。安装：`hermes skills tap add witt3rd/oh-my-hermes`。
...
- [Hermes 多代理系统](https://github.com/sergiocoding96/hermes-multi-agent) —— Paperclip + Hermes + MemOS 多代理系统，具备自动研究自改进循环、研究技能和会话持久化。
...
- [从 docker-compose 到 Helm：在 Amazon EKS 上部署 Hermes Agent](https://ashkmb.com/blog/hermes-agent-eks-deployment/) —— 生产级 EKS 部署实战文章，涵盖私有 ECR、IRSA、API 密钥管理、持久卷与自定义 Helm Chart。
- [Hermes Desktop Guide（日本語）](https://github.com/takunagai/hermes-desktop-guide) —— 非官方的 Hermes Desktop 日语指南，基于 Astro Starlight 构建，支持 Obsidian 内容管理、Pagefind 搜索和 Cloudflare 部署。
