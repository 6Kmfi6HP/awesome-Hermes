...
- [Hermes Paperclip Cockpit](https://github.com/4crimson/hermes-paperclip-cockpit) — plugin that turns Telegram into a deterministic control surface for Paperclip, with slash-command views, natural-language rewrite hooks, and conservative safety defaults that keep Paperclip control out of the model context.
- [Hermes Plugin: Obscura Browser](https://github.com/SGavrl/hermes-plugin-obscura) — lightweight browser backend that runs Hermes browser tasks on the Rust-based Obscura headless browser via CDP, ~70 MB binary and ~30 MB RAM per session with no Chrome or Node.js dependency.
- [Hermes Forge](https://github.com/iknowkungfubar/hermes-forge) — self-hosted MCP server and Hermes plugin that brings the Forge guardrail framework to Hermes: response validation, rescue parsing, step enforcement, and context management for safer LLM tool-calling.
- [Sensorium](https://github.com/TKCen/sensorium) — environment-reactive attention substrate plugin that gives Hermes a bounded inner-lifecycle layer: salience sensing, subconscious/advisory review, conscious thread capsules, and safe action preparation without autonomous outbound behavior.
- [Shark Trading Agent](https://github.com/logiqfish/shark-trading-agent) — skinny, self-hosted paper-only autonomous trading bot that runs as a Hermes Agent profile distribution, using Alpaca paper API for market data and execution. Two keys (Alpaca paper + an LLM), no hosted services, educational.
...
- [Hermes Offline Installer](https://github.com/bddiudiu/hermes-offline-installer) — offline installer builder that packages Hermes Agent with a portable Python runtime, uv, wheels, dashboard assets, skills, and plugins into one-click installers for Windows, macOS, and Linux, with no GitHub or PyPI network access required during installation.
